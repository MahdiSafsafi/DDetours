...
my @legacy_ins = (
	...
	'iret',  'enter', 'leave', 'int3',   'inc',   'dec',
	'mul',   'imul',  'div',   'idiv',   'push',  'pop',
	'pusha', 'popa',  'pushf', 'popf',   'test', 'nop', 'xchg',
	'mov',   'cmp',   'lea',   'wait',   
);
open( F, 'x86.map' );
...
while (<F>) {
	...
	if (/(Grp)*Table:\s*(.+)$/) {
		...
		for ( my $i = 0 ; $i < $max + 1 ; $i++ ) {
			my $op = sprintf( "%.2X", $i );
			$hash{$i} = "(INST_ID_INVALID shl \$08) or OPF_INVALID {0x$op: InvalidOpCode }";
		}
	}
	if (/EndTable/) {
		my @table;
